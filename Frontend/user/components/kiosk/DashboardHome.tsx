...
const DashboardHome: React.FC<Props> = React.memo(({ alerts, onNavigate, userName = "Citizen", language }) => {
    ...
    const greetingSection = (
        <motion.div variants={itemVariants} className={`flex ${isVertical ? 'flex-col gap-6 p-8' : 'justify-between items-center p-6'} bg-white rounded-[2.5rem] shadow-sm border border-slate-100`}>
            <div className={isVertical ? 'text-center' : ''}>
                <h2 className={`${isVertical ? 'text-4xl' : 'text-3xl'} font-black text-slate-900 tracking-tight`}>
            ...
            {wardContacts.length > 0 && (
                <div className={`bg-slate-50 px-6 py-4 rounded-[1.5rem] flex items-center gap-4 ${isVertical ? 'w-full justify-center border border-slate-100' : 'border border-slate-200'}`}>
    ...
    const alertsCenterSection = (
        <div className={`bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden flex flex-col ${isVertical ? 'min-h-[500px]' : 'h-full'}`}>
            <div className={`border-b border-slate-100 bg-slate-50/50 flex justify-between items-center ${isVertical ? 'p-8' : 'p-6'}`}>
                <div>
                    <h3 className={`${isVertical ? 'text-2xl' : 'text-xl'} font-black text-slate-900 flex items-center gap-2`}>
            ...
            <div className={`flex-1 overflow-y-auto custom-scrollbar ${isVertical ? 'p-6' : 'p-4'}`}>
    ...
    const publicNoticeBoardSection = (
        <div className={`bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden flex flex-col ${isVertical ? 'min-h-[500px]' : 'h-full'}`}>
            <div className={`border-b border-slate-100 bg-slate-50/50 ${isVertical ? 'p-8' : 'p-6'}`}>
    ...
    const importantContactsSection = (
        <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
            ...
            <div className={isVertical ? 'p-8' : 'p-6'}>
                <div className={`grid ${isVertical ? 'grid-cols-2 gap-6' : 'grid-cols-3 xl:grid-cols-6 gap-4'}`}>
    ...
    return (
        <motion.div variants={containerVariants} initial="hidden" animate="show" className={`max-w-7xl mx-auto pb-10 ${isVertical ? 'px-8 pt-8 space-y-8' : 'px-4 pt-4 space-y-6'}`}>
            {/* Section 1: Welcome & Support */}
            {greetingSection}

            {/* Section 2: Expanded Live City Map */}
            <motion.div variants={itemVariants} className="w-full">
                <div className={`bg-white rounded-[2.5rem] shadow-sm border border-slate-100 ${isVertical ? 'p-6' : 'p-4'}`}>
                    <div className="mb-6 px-4 flex justify-between items-end">
                        <div>
                            <h3 className={`${isVertical ? 'text-3xl' : 'text-2xl'} font-black text-slate-900 flex items-center gap-3`}>
                                <MapPin className="text-blue-500" size={isVertical ? 32 : 28} />
                                Live City Map
                            </h3>
                            <p className={`text-slate-500 font-medium mt-1 ${isVertical ? 'text-lg' : 'text-sm'}`}>Locality awareness & city-wide visibility</p>
                        </div>
                    </div>
                    {/* DisruptionMap handles its own container size natively, but we give it a wrapper */}
                    <div className={`w-full ${isVertical ? 'h-[600px] lg:h-[700px]' : 'h-[400px] lg:h-[500px]'} rounded-[2rem] overflow-hidden`}>
            ...
            <motion.div variants={itemVariants} className={`grid grid-cols-1 xl:grid-cols-2 items-stretch ${isVertical ? 'gap-8' : 'gap-6'}`}>
