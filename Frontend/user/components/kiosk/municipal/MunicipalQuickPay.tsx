...
const MunicipalQuickPay: React.FC<Props> = ({ onBack, language }) => {
    ...
    useEffect(() => {
        const savedSession = sessionStorage.getItem('aazhi_last_receipt_muni');
        if (savedSession) {
            try {
                ...
                sessionStorage.removeItem('aazhi_last_receipt_muni');
                setStep('INPUT');
            }
        } else {
            setConsumerNo('');
    ...
    const handlePaymentSuccess = (paymentId: string) => {
        ...
        sessionStorage.setItem('aazhi_last_receipt_muni', JSON.stringify({
            consumerNo,
            billData: billData,
                        ...
                        <div className="bg-slate-900 p-8 text-white flex justify-between items-start">
                            <div>
                                ...
                                <h2 className="text-5xl font-black tracking-tight flex items-baseline gap-1">
                                    ...
                                    {Number(billData.amount || 0).toFixed(2)}
                                </h2>
                            </div>
            ...
            {step === 'PAYMENT' && billData && (
                <div className="space-y-6 animate-in slide-in-from-right-8">
                    <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-100 text-center">
                        <h3 className="text-xl font-black text-slate-900 mb-6">{t('paymentMode') || "Secure Payment Gateway"}</h3>
                        ...
                        <div className="bg-slate-50 p-8 rounded-3xl mb-8 border border-slate-100">
                            ...
                            <p className="text-5xl font-black text-slate-900 mb-6">₹{Number(billData.amount || 0).toFixed(2)}</p>
                ...
                <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-slate-100 text-center animate-in zoom-in-95">
                    ...
                    <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 text-left space-y-3 mb-8">
                        ...
                        <div className="flex justify-between">
                            ...
                            <span className="text-sm font-black text-cyan-600">₹{Number(billData.amount || 0).toFixed(2)}</span>
                        </div>
                    </div>
            ...
            {step === 'SUCCESS' && billData && paymentRef && (
                <OfficialReceipt
                    data={{
                        ...
                        consumerName: billData.metadata?.consumer_name_masked || 'Unknown',
                        billAmount: `Rs. ${Number(billData.amount || 0).toFixed(2)}`,
                        billMonth: billData.billing_month,
                        receiptNo: `WR-${Date.now().toString().slice(-8)}`,
                        dateTime: new Date().toLocaleString(),
                        debitAmount: `Rs. ${Number(billData.amount || 0).toFixed(2)}`,
