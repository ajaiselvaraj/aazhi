...
const GasQuickPay: React.FC<Props> = ({ onBack, language }) => {
  ...
  const handleFetchBill = async (e: React.FormEvent) => {
    ...
    try {
      const fetchedBill = await GasService.getQuickPayBill(consumerId);
      setBill(fetchedBill);
    } catch (err: any) {
      setError(err.response?.data?.message || 'Failed to fetch bill. Please check the Consumer ID.');
      setBill(null);
    } finally {
      setLoading(false);
    }
  ...
  if (paymentSuccess) {
    return (
      <div className="max-w-2xl mx-auto p-12 bg-white rounded-[3rem] shadow-xl border border-slate-100 text-center relative top-10">
        ...
        <p className="text-slate-500 text-lg mb-8">Your gas bill of ₹{bill?.total_amount} has been securely paid.</p>
        <button 
          onClick={onBack}
          className="px-8 py-4 bg-orange-500 hover:bg-orange-600 active:scale-95 text-white rounded-2xl font-bold text-lg transition-all"
        >
          Return to Dashboard
        </button>
      </div>
      ...
      <div className="bg-white rounded-[2rem] shadow-xl p-10 border border-slate-100">
        ...
        <form onSubmit={handleFetchBill} className="flex gap-4 mb-8">
          <input
            ...
            className="flex-1 bg-slate-50 border border-slate-200 px-6 py-4 rounded-2xl text-lg font-bold outline-none focus:ring-4 focus:ring-orange-500/20 focus:border-orange-500 transition-all"
          />
          <button 
            type="submit" 
          ...
          <div className="border border-slate-200 rounded-3xl p-8 bg-slate-50">
            <div className="flex justify-between items-start mb-6 pb-6 border-b border-slate-200">
              <div>
                ...
                <p className="text-xl font-black">{bill.metadata?.consumer_name_masked || 'A*** K****'}</p>
              </div>
            ...
            <div className="flex justify-between items-end mb-8">
              <div>
                ...
                <p className="text-5xl font-black tracking-tight">₹{bill.total_amount}</p>
