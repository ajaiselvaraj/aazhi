...
export interface SuvidhaVoiceControlProps {
  ...
  /**
   * Whether to start listening automatically on mount
   */
  autoStart?: boolean;
}
...
const getBcp47Locale = (lang: string, defaultLocale = 'en-IN') => {
  ...
  return map[lang.toLowerCase()] || defaultLocale;
};

// ─── Component ────────────────────────────────────────────────────
const SuvidhaVoiceControlImpl = forwardRef<SuvidhaVoiceControlRef, SuvidhaVoiceControlProps>(
  (
    {
      onCommand,
      ttsLanguage = 'English',
    ...
    // Sync internal lang state when parent prop changes (e.g. user picks a language)
    useEffect(() => {
...
SuvidhaVoiceControlImpl.displayName = 'SuvidhaVoiceControlImpl';
...
export default SuvidhaVoiceControlImpl;
