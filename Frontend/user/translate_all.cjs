...
const TARGET_LANGS = ['kn', 'ml', 'mr', 'or', 'pa', 'doi', 'ks', 'kok', 'mai', 'sa', 'sat', 'sd'];
...
async function runGeno() {
    ...
    for (const lang of TARGET_LANGS) {
        ...
        if (keysToTranslate.length === 0) {
            console.log(`Skipping ${lang}, already fully translated.`);
            continue;
        }
        ...
        for (let i = 0; i < keysToTranslate.length; i += CHUNK_SIZE) {
            ...
            while (!success && retries < MAX_RETRIES) {
                 try {
                     ...
                     res.forEach((r, idx) => {
                         currentObj[chunkKeys[idx]] = r.text;
                     });
                     success = true;
                     ...
                     retries++;
                     const waitTime = retries >= 3 ? 30000 : 5000 * retries;
                     console.log(`[${lang}] Try ${retries} failed. Retrying in ${waitTime/1000}s...`);
                     if (retries >= MAX_RETRIES) {
                         console.error(`[FATAL] Skipping chunk.`);
                     } else {
